...
4. 点击“保存到本地浏览器”，下次打开会自动恢复这些信息。
5. 如需填写起止时间和地点，可直接在报销单表格内编辑，并可新增行。
6. OCR 回填后请人工检查金额、时间、地点是否正确。
7. 点击“打印”进入独立打印页，再确认打印。

## 报销单信息缓存

以下字段可点击“保存到本地浏览器”后保存在浏览器本地：
...
缓存使用 `localStorage.invoiceExpenseForm`。填写时会立即更新当前预览，但只有点击保存按钮才会写入浏览器本地缓存。如果要清空，可以在浏览器开发者工具中删除该项。
